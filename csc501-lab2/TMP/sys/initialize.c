...
//CHANGES
bs_map_t bsm_tab[NBSM_TAB];
fr_map_t frm_tab[NFRAMES];
unsigned int pick_pid;
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pt_t *row;
	unsigned int index = 0, tbl_row = 0;
	for(index = 0; index < 4; index++) {
		frm_tab[index].fr_status = FRM_MAPPED;
		frm_tab[index].fr_type = FR_TBL;
		frm_tab[index].fr_pid = NULLPROC;
		frm_tab[index].fr_refcnt = 1;
		row = (pt_t *)((FRAME0 + index)*NBPG);
		for(tbl_row = 0; tbl_row < 1024; tbl_row++) {
			row[tbl_row].pt_pres = 1;
			row[tbl_row].pt_write = 1;
			row[tbl_row].pt_user = 1;
			row[tbl_row].pt_pwt = 0;
			row[tbl_row].pt_pcd = 0;
			row[tbl_row].pt_acc = 0;
			row[tbl_row].pt_dirty = 0;
			row[tbl_row].pt_mbz = 0;
			row[tbl_row].pt_global = 1;
			row[tbl_row].pt_base = index*FRAME0 + tbl_row;
			row[tbl_row].pt_avail = 0;
		}
	}
	init_pagedir(NULLPROC);
